...
net_file = './yolov6n_deploy.prototxt'
caffe_model = './yolov6n_deploy.caffemodel'
...
def postprocess(out, img_h, img_w):
    ...
    output.append(out['conv_blob49'].reshape((-1)))  # cls
    output.append(out['conv_blob51'].reshape((-1)))  # reg

    output.append(out['conv_blob54'].reshape((-1)))  # cls
    output.append(out['conv_blob56'].reshape((-1)))  # reg
            ...
            for w in range(mapSize[index][1]):
                ...
                for cl in range(class_num):
                    ...
                    if cls_val > objectThresh:
                        ...
                        x2 = (meshgrid[gridIndex + 0] + reg[0 * mapSize[index][0] * mapSize[index][1] + h * mapSize[index][1] + w]) * strides[index]
                        y2 = (meshgrid[gridIndex + 1] + reg[1 * mapSize[index][0] * mapSize[index][1] + h * mapSize[index][1] + w]) * strides[index]
...
def detect(img_path):
    ...
    for i in range(len(predbox)):
        ...
        cv2.putText(orig, title, ptext, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)

    cv2.imwrite('./test_caffe_result.jpg', orig)
